    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (axiosErr.response) {
          if (isLogin && axiosErr.response.status === 401) {
            toast.error('Invalid email or password.', { position: 'top-center' });
          } else if (axiosErr.response.status === 409) {
            toast.error(axiosErr.response.data?.error || 'Email or username already exists.', { position: 'top-center' });
          } else if (axiosErr.response.status === 500) {
            toast.error('Server is down or not reachable.', { position: 'top-center' });
          } else {
            toast.error(axiosErr.response.data?.error || 'Request failed. Please try again.', { position: 'top-center' });
          }
              ...
              <div className="p-6">
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    disabled={loading}
                  >
                    {loading ? (isLogin ? 'Signing in...' : 'Creating...') : (isLogin ? t.loginButton : t.signupButton)}
                  </button>
                </form>
